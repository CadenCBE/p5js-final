...
//CONTROLS
    ...
    //player collision
    if(ball1.x - (ball1.size / 2) < p1.x && ball1.y < p1.y + (p1.height/2) && ball1.y > p1.y - (p1.height/2)){
        ball1.xSpeed *= -1
    }
    if(ball1.x + (ball1.size / 2) > p2.x && ball1.y < p2.y + (p2.height/2) && ball1.y > p2.y - (p2.height/2)){
        ball1.xSpeed *= -1
    }
}
...
function keyPressed(){
    if(keyCode === 13 && gameStarted == true){ // enter
        if(ballInPlay == false){
            ballInPlay = true;
        }
        ball1.flash = false;
